...
<div class="user-container">
    ...
    <div class="user-subscription">
      ...
      <div class="sub-form">
       ...
       <div class="sub-text">@if(!blank($transaction) && !blank($transaction->created_at)) 
        @if($plan->id == '2')
        Your plan will automatically renew on <strong>{{date('d.m.Y',strtotime('+30 days',strtotime($transaction->created_at)))}} </strong> and you'll be charged <strong>${{$plan->amount}}</strong> 
        
        @elseif($plan->id == '3')
        Your plan will automatically renew on <strong> {{date('d.m.Y',strtotime('+365 days',strtotime($transaction->created_at)))}} </strong> and you'll be charged <strong>${{$plan->amount}}</strong>

         @elseif($plan->id == '4') <strong>Unlimited benefits </strong> <strong>${{$plan->amount}}</strong> @else <strong> No benefits </strong>
